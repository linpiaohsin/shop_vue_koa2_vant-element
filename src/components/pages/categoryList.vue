            ...
            <van-pull-refresh v-model = 'isRefresh' @refresh = 'onRefresh'>
              <van-list
                ...
                <div class="itemList" v-for="(item,index) in goodsList" :key='index' @click="goGoodsDetail(item.ID)">
                  <div class="imgList">
                    <img :onerror="errImg" v-lazy="item.IMAGE1" width="100%"  >
                    <!-- <img :onerror="errImg" v-lazy="item.IMAGE1" width="100%"  > -->
                  </div>
                  <div class="textList">
                    <div>{{item.NAME}}</div>
                    <div>¥{{item.PRESENT_PRICE|resetMoney}}</div>
                  </div>
                </div>
              </van-list>
            </van-pull-refresh>
...
<script>
  ...
  import {SetMoney} from '@/filters/Money.js'
	export default {
		...
		data() {
			return {
				...
				list:{
						...
						error: '../../assets/img/null-password.jpg',
						loading: '../../assets/img/null-password.jpg'
				},
				categoryList:[],
				cateIndex:0,
				categorySubList:[],
        ...
        goodsList:[],
        categorySubId:'',
        errImg:'this.src="'+ require('@/assets/img/errImg.png')+'"',
